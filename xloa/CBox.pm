...
# Creates a window with a question
...
#---------------------
# Shows a text and two buttons at the center of the board - while the box is
# shown all other actions are blocked
...
sub show_question {
	...
	my $box = $mw->Toplevel( '-title' => "Xloa: Offer to give up" );
	...
	$frame1->Label( '-text' => "Loa offers to give up.\nDo you accept ?",
					'-font' => $opt{ menufont }
				  )->pack ( '-pady' => '4m',
							'-padx' => '4m' );
	...
	$mw->bind( 'all', '<Escape>', sub { $mess_end = -1 } );
	...
	# Wait for the Yes or No button to be pressed
